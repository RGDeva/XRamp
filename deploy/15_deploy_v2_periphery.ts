...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!spokePoolAddress) {
    if (network === "localhost" || network === "hardhat") {
      ...
      console.log(`No Across SpokePool configured for ${network} — skipping AcrossBridgeHookV2 deployment`);
    }
  }

  // Deploy AcrossBridgeHookV2 only if a SpokePool is available
  let acrossBridgeHookV2Address: string | undefined;
  if (spokePoolAddress) {
    const acrossBridgeHookV2 = await deploy("AcrossBridgeHookV2", {
      from: deployer,
      args: [usdcAddress, orchestratorRegistryAddress, spokePoolAddress],
    });
    acrossBridgeHookV2Address = acrossBridgeHookV2.address;
    console.log("AcrossBridgeHookV2 deployed at", acrossBridgeHookV2Address);
    await waitForDeploymentDelay(hre);
  }
  ...
  // Register AcrossBridgeHookV2 in PostIntentHookRegistry (only if deployed)
  const postIntentHookRegistry = await ethers.getContractAt("PostIntentHookRegistry", postIntentHookRegistryAddress);
  if (acrossBridgeHookV2Address) {
    await addPostIntentHook(hre, postIntentHookRegistry, acrossBridgeHookV2Address);
  ...
  if (acrossBridgeHookV2Address) {
    const acrossBridgeHookV2Contract = await ethers.getContractAt("AcrossBridgeHookV2", acrossBridgeHookV2Address);
...
func.skip = async (hre: HardhatRuntimeEnvironment): Promise<boolean> => {
  ...
  if (network !== "localhost") {
    ...
    // Skip if V2 system isn't deployed yet
    try {
      getDeployedContractAddress(hre.network.name, "OrchestratorRegistry");
    } catch (e) {
      console.log(`Skipping V2 periphery deploy: OrchestratorRegistry not found on ${network}`);
      return true;
    }
    return false;
  }
