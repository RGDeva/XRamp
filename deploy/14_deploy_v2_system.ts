...
func.skip = async (hre: HardhatRuntimeEnvironment): Promise<boolean> => {
  ...
  if (network != "localhost") {
    // Skip if V2 is already deployed
    try { getDeployedContractAddress(hre.network.name, "OrchestratorV2"); return true; } catch (e) {}
    // Skip if V1 Orchestrator isn't deployed yet (dependency not met)
    try { getDeployedContractAddress(hre.network.name, "Orchestrator"); } catch (e) {
      console.log(`Skipping V2 deploy: V1 Orchestrator not found on ${network}`);
      return true;
    }
    // Skip if SimpleAttestationVerifier isn't deployed yet (01 dependency not met)
    try { getDeployedContractAddress(hre.network.name, "SimpleAttestationVerifier"); } catch (e) {
      console.log(`Skipping V2 deploy: SimpleAttestationVerifier not found on ${network}`);
      return true;
    }
    return false;
  }
