...
func.skip = async (hre: HardhatRuntimeEnvironment): Promise<boolean> => {
  ...
  if (network !== "localhost" && network !== "hardhat") {
    // Skip if V2 verifier isn't deployed yet
    try {
      getDeployedContractAddress(hre.network.name, "UnifiedPaymentVerifierV2");
    } catch (e) {
      console.log(`Skipping V2 payment method configuration: UnifiedPaymentVerifierV2 not found on ${network}`);
      return true;
    }
  }
  return false;
